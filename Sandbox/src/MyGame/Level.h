...
public:
    ...
    uint32_t XToColumn(float x) const { return std::min(m_Width - 1, (uint32_t)(x / m_TileSize)); }
    uint32_t YToRow(float y) const { return std::min(m_Height - 1, (uint32_t)(y / m_TileSize)); }
...
private:
	...
	float m_TileSize = 75.0f;

	uint8_t m_Map[MAX_LEVEL_SIZE][MAX_LEVEL_SIZE];
	Ref<Tile> m_Tiles[(uint32_t)GameObjectType::Count];
